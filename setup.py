# Copyright 2020 The fastcpd/CLODE/TensorStore Authors
#
...
SYSTEM_PYTHON_LIBS_ENVVAR = 'FASTCPD_SYSTEM_PYTHON_LIBS'
...
class BuildExtCommand(setuptools.command.build_ext.build_ext):
    ...
    def run(self):
        if not self.dry_run:
            ...
            prebuilt_path = os.getenv('FASTCPD_PREBUILT_DIR')
            if os.name == 'nt':
                suffix = '.dll'
                prefix = ''
            else:
            ...
            if not prebuilt_path:
                ...
                built_ext_path = os.path.join(
                    "bazel-bin", fastcpd_lib_file)
            else:
                # If `FASTCPD_PREBUILT_DIR` is set, the extension module is assumed
                # to have already been built a prior call to `build_ext -b
                # $FASTCPD_PREBUILT_DIR`.
                #
